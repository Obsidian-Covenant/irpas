...
int main(int argc, char **argv) {
    ...
    int			psize;
    int			rc,allrespond,respond=0,reached=0;
	socklen_t addrsize;
    uint16_t		TTL;
    uint32_t		tseq;
	...
	for (i=0;i<cfg.probes;i++) {
	    ...
	    tpacket=(uint8_t *)smalloc(sizeof(tcphdr_t)+sizeof(iphdr_t)+3);
	    /* bulid IP packet */
	    ip=(iphdr_t *)tpacket;
	    ip->version=4;
	    ...
	    tcp->th_sum=chksum((uint8_t *) &pshdr,
		...
		if ((rc=recvfrom(rfd,(uint8_t *)tpacket,psize,0,
			...
			if (cfg.verbose) {
				...
				printf("Should be %zu, is %d\n",
				       (size_t)(2*sizeof(iphdr_t)+sizeof(icmphdr_t)+4),
				       rc);
			}
		    }
		...
		if ((rc=recvfrom(tcfd,(uint8_t *)tpacket,psize,0,
			    (struct sockaddr *)&from,
			...
			printf("FUCK: %d %zu\n", rc,
			       (size_t)(sizeof(tcphdr_t)+sizeof(iphdr_t)));
