...
uint8_t	*construct_icmp_redirect(struct in_addr *dest,
	struct in_addr *newgw, int *psize,
	uint8_t *iporig, unsigned int iporig_length);
...
int	main(int argc, char **argv) {
	...
	switch (option) {
	    ...
	    case 'd':	inet=smalloc(strlen(optarg)+1);
			...
			cfg.tnet=ntohl(*((uint32_t*)&(temp.s_addr)));
			if (!inet_aton(imask,&temp)) {
			    fprintf(stderr,"Target netmask %s incorrect\n",
				    inet);
			    return (1);
			}
			cfg.tmask=ntohl(*((uint32_t*)&(temp.s_addr)));
			/* make the net flat ;) */
	    ...
	    case 's':	inet=smalloc(strlen(optarg)+1);
			...
			cfg.snet=ntohl(*((uint32_t*)&(temp.s_addr)));
			if (!inet_aton(imask,&temp)) {
			    fprintf(stderr,"Source netmask %s incorrect\n",
				    inet);
			    return (1);
			}
			cfg.smask=ntohl(*((uint32_t*)&(temp.s_addr)));
			/* make the net flat ;) */
			cfg.snet=cfg.snet&cfg.smask;
			free(inet);
			break;
	    case 'w':	cfg.delay=(time_t)atoll(optarg);
			break;
...
void	evaluate_packet(uint8_t *frame,int frame_length) {
	...
	if ( 
		((ntohl(*((uint32_t*)&(ip->daddr.s_addr)))&cfg.tmask)
		 ==cfg.tnet)
		&&
		((ntohl(*((uint32_t*)&(ip->saddr.s_addr)))&cfg.smask)
		 ==cfg.snet)
		){
	    if (cfg.verbose>1) {
	    ...
	    if (
		    (c==NULL)
		    ||
		    ((c->t+cfg.delay)<time(NULL))
		    ){
		icp=construct_icmp_redirect(&(ip->saddr),&(cfg.gw),&icl,
			(uint8_t *)ip,frame_length-sizeof(struct ether_header));
		...
		printf("  Traffic path redirected %lld secs ago."
			" Keeping silence.\n",
			(long long int)(time(NULL)-c->t));
	} else return;
    } /* not IP */ 
}
...
uint8_t	*construct_icmp_redirect(struct in_addr *dest,
    ...
    *psize=sizeof(icmp_redirect_t)+sizeof(iphdr_t);
    tpacket=(uint8_t *)smalloc(sizeof(icmp_redirect_t)+sizeof(iphdr_t)
    ...
    /* make up checksum */
    cs=chksum((uint8_t *)icmp,sizeof(icmp_redirect_t));
    icmp->checksum=cs;
