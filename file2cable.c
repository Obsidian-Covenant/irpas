...
int main(int argc,char **argv) {
    ...
    if (stat(cfg.filename,&sbuf)!=0) {
        ...
        ssize_t nread = read(fd, buffer, (size_t)sbuf.st_size);
        if (nread < 0) {
            perror("read()");
            close(fd);
            exit(1);
        ...
        if (nread != sbuf.st_size) {
            fprintf(stderr, "read(): expected %ld bytes, got %ld\n",
                    (long)sbuf.st_size, (long)nread);
            close(fd);
            exit(1);
        }
	      close(fd);
