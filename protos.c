...
int recv_icmp(pid_t pid) {
	...
	if (recvfrom(rfd,(uint8_t *)response,rlength,0,
		...
		if (icmp->code==ICMP_UNREACH_PROTO) {
			...
			if ((!foundflag)&&(cfg.verbose>2))
			    printf("response from %s is not related"
				    " to a probe\n",
				    inet_ntoa(from.sin_addr));
		    }
...
#define PING_ROUND	3
	...
	while ((!cfg.dontping)&&
	    ...
	    tpacket=(uint8_t *)smalloc(sizeof(icmp_ping_t)+64);
            pident=0xAF0D;
	    ...
	    pingh->icmp.checksum=chksum((uint8_t *)pingh,psize);
	    ...
	    if ((rc=recvfrom(sfd,(uint8_t *)tpacket,psize,0,
