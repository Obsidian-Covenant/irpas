...
/* IGRP construction */
uint8_t	*construct_igrp(int from, int to, uint16_t autosys, int *psize);
...
void	send_table(int autosys) {
	...
	printf("Routing table size (%zu octets) is too large for one packet\n"
	       "Splitting up ...\n",
	       sizeof(igrp_system_entry_t) * (size_t)routesc);
...
uint8_t	*construct_igrp(int from, int to, uint16_t autosys, int *psize) {
    ...
    *psize=sizeof(igrp_system_entry_t)*(to-from)+
    ...
    tpacket=(uint8_t *)smalloc(sizeof(igrp_system_entry_t)*(to-from)+
	    sizeof(igrp_t)+sizeof(iphdr_t)
...
#define IP_DELIMITER	'.'
	...
	memcpy((uint8_t *)&(routes[routesc].destination[0]),
		(uint8_t *)&(taddr.s_addr),3);
	...
	lt=htonl(atol(lp));
	memcpy(&(routes[routesc].delay),((uint8_t *)&lt)+1,3);

	/* get the bandwidth */
	if ((lp=strchr(lp2,DELIMITER))==NULL) {
	    fprintf(stderr,"incomplete line in routing file (bandwidth)\n");
	    return (-1);
	}
	lp[0]='\0'; lp++;
	//routes[routesc].bandwith=htons(atol(lp2));
	lt=htonl(atol(lp2));
	memcpy(&(routes[routesc].bandwith),((uint8_t *)&lt)+1,3);
...
void	print_routing_table(void) {
    ...
    printf("Destination | Delay | Bandwidth | "
	    " MTU  | Reliability | Load | Hop count\n");
    for (i=0;i<routesc;i++) {

	memcpy(((uint8_t *)&lt)+1,&(routes[i].bandwith),3);
	memcpy(((uint8_t *)&lt2)+1,&(routes[i].delay),3);

	printf("%03d.%03d.%03d | %5d | %9d | %5d | %11d | %4d | %9d \n",
