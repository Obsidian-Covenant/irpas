...
uint8_t	*construct_arp_frame(int *fsize, struct in_addr *dest);
uint8_t	*construct_dhcp_discover_frame(int *fsize,
	struct in_addr *saddr,lease_t *l);
uint8_t	*construct_dhcp_request_frame(int *fsize,
	struct in_addr *saddr,lease_t *l);

void	arp_add(struct in_addr *addr, uint8_t *hwaddr);
uint8_t	*arp_find(struct in_addr *addr);
...
lease_t *server_find_lease(server_t *s,uint8_t *hwaddr,unsigned long xid);
void	server_update_lease(struct in_addr *srvaddr,
	uint8_t *hwaddr, unsigned long xid, 
	...
	if (ip->protocol==IPPROTO_UDP) {
		...
		do {
			...
			dhcp_renew=(unsigned long)ntohl(
				*((uint32_t *)&(dhcpo->value)));
		    }

		    dhcpo=(dhcp_option_t *)
			((uint8_t*)dhcpo+dhcpo->length+2);
		} while (
			(((uint8_t*)dhcpo-frame)<frame_length) // length 
		...
		if (dhcpop==DHCPOFFER) {
			...
			arp_add(&serverid,(uint8_t *)&(eth->ether_shost));
			server_add(&serverid);
		    } else {
			server_update_lease(&serverid,
				(uint8_t *)&(dhcp->chaddr),xid,
...
uint8_t	*construct_arp_frame(int *fsize, struct in_addr *dest) {
    ...
    *fsize=sizeof(arphdr_t)+sizeof(struct ether_header);
    packet=(uint8_t *)smalloc(*fsize+3);
...
#define DSIZE	300			// empiric value ?!?
    ...
    packet=(uint8_t *)smalloc(DSIZE+sizeof(struct ether_header)+3);
    *fsize=DSIZE+sizeof(struct ether_header);
    ...
    cs=chksum((uint8_t *)ip,sizeof(iphdr_t));
    ip->check=cs;

    /* UDP header */
    udp=(udphdr_t *)((uint8_t *)ip+sizeof(iphdr_t));
    udp->sport=htons(DHCP_CLIENT_PORT);
    udp->dport=htons(DHCP_SERVER_PORT);
    udp->length=htons(DSIZE-sizeof(iphdr_t));

    /* BOOTP header */
    dhcp=(dhcp_t *)((uint8_t *)udp+sizeof(udphdr_t));
    dhcp->msgtype=1;
    dhcp->hwtype=1;
    dhcp->hwalen=6;
    dhcp->hops=0;
    if (l==NULL)
	dhcp->transid=(uint32_t)htonl(
		1+(unsigned long)(4294967293.0*rand()/(RAND_MAX+1.0)));
    else
	dhcp->transid=(uint32_t)htonl(l->xid);
    ...
    /* DHCP options */
    dhcpo=(dhcp_option_t *)((uint8_t *)dhcp+sizeof(dhcp_t));
    dhcpo->type=53; dhcpo->length=1; dhcpo->value=DHCPDISCOVER;

    /* next option ... (PARAMETERS) */
    dhcpo=(dhcp_option_t *)((uint8_t *)dhcpo+((dhcp_option_t *)dhcpo)->length+2);
...
#define SIZE	(sizeof(iphdr_t)			\
    ...
    packet=(uint8_t *)smalloc(SIZE+sizeof(struct ether_header)+3);
