...
/* IGRP construction */
uint8_t	*construct_igrp_request(uint16_t autosys, int *psize);
/* EIGRP construction */
uint8_t	*construct_eigrp_request(struct in_addr *dd, 
	uint32_t autosys, int *psize);
/* IRDP construction */
uint8_t	*construct_irdp_request(int *psize);
/* RIP construction */
uint8_t	*construct_rip_request(int *psize);
uint8_t	*construct_rip2_request(int *psize);

/* PCAP */
int     initialize_pcap(void);
void	signaler(int sig);
void	evaluate_packet(uint8_t *frame,int frame_length);
...
void	evaluate_packet(uint8_t *frame,int frame_length) {
	...
	do {
		...
		if (cfg.verbose>2) 
			...
			ntohl(*((uint32_t *)
				((uint8_t*)cdpg+2*sizeof(uint16_t)))
			    ));

		cdpa=(struct cdp_address_entry *)((uint8_t*)cdpg+
		...
		if (cdpa->proto!=0xCC) {
		    if (cfg.verbose) printf("CDP not speaking IP at"
			    ...
			    *((uint8_t*)frame+10)&0xFF,
			    *((uint8_t*)frame+11)&0xFF);
		    memset(&(caa),0,sizeof(caa));
		    add_route(CAP_CDP,&caa,cdpdatalength,(uint8_t*)cdpbegin); 
		} else {
	    ...
	    cdpg=(cdp_generic_t *)((uint8_t*)cdpg+ntohs(cdpg->length));

	} while (((uint8_t*)cdpg-(uint8_t*)frame)<frame_length); 
...
#define PADDING_SIZE	14
    ...
    *psize=PADDING_SIZE+sizeof(igrp_t)+sizeof(iphdr_t);
    tpacket=(uint8_t *)smalloc(PADDING_SIZE+sizeof(igrp_t)+sizeof(iphdr_t)
    ...
    /* make up checksum */
    cs=chksum((uint8_t *)igrph,(*psize-sizeof(iphdr_t)));
    igrph->checksum=cs;
...
uint8_t	*construct_eigrp_request(struct in_addr *dd, 
    ...
    *psize=sizeof(iphdr_t)+sizeof(eigrp_t)
    ...
    /* make up checksum */
    cs=chksum((uint8_t *)eigrph,(*psize-sizeof(iphdr_t)));
    eigrph->checksum=cs;
...
uint8_t	*construct_irdp_request(int *psize) {
    ...
    *psize=sizeof(irdp_solicitation_t)+sizeof(iphdr_t);
    tpacket=(uint8_t *)smalloc(sizeof(irdp_solicitation_t)+sizeof(iphdr_t)
	    +3 /* for my checksum function, which sometimes 
		  steps over the mark */
	    );
    ...
    /* make up checksum */
    cs=chksum((uint8_t *)irdph,(*psize-sizeof(iphdr_t)));
    irdph->checksum=cs;
...
uint8_t	*construct_rip_request(int *psize) {
    ...
    *psize=sizeof(udphdr_t)+sizeof(iphdr_t)+
    ...
    tpacket=(uint8_t *)smalloc(sizeof(udphdr_t)+sizeof(iphdr_t)+
	    sizeof(ripv1hdr_t)+sizeof(ripv1addr_t)
	    +3 /* for my checksum function, which sometimes 
		  steps over the mark */
	    );
...
int	add_route(int rtype,struct in_addr *addr,unsigned int as,void *data) {
	...
	case CAP_IGRP: /* add an IGRP route */
		...
		memcpy(((uint8_t *)&temp)+1,&(isys->delay),3);
		c_igrp->delay=ntohl(temp);
		memset(&temp,0,sizeof(temp));
		memcpy(((uint8_t *)&temp)+1,&(isys->bandwith),3);
	...
	case CAP_EIGRP_RT:
		...
		memset(&temp,0,sizeof(temp));
		memcpy(((uint8_t *)&temp)+1,&(eigrper->mtu),3);
		...
		memset(&temp,0,sizeof(temp));
		memcpy(((uint8_t *)&temp)+1,&(eigrpir->mtu),3);
		c_eigrpr->mtu=ntohs(temp);
	...
	case CAP_OSPF:
		...
		c_ospf->dead=ntohl(*((uint32_t *)&(ospfl->dead_interval)));
	...
	case CAP_CDP:
		...
		if (ntohs(cdprec->type)==TYPE_DEVICE_ID) {
		    ...
		    c_cdp->vlan=ntohs(*((uint16_t*)&(cdprec->value)));
		} else if (ntohs(cdprec->type)==TYPE_DUPLEX) {
		    c_cdp->duplex=(uint8_t)cdprec->value;
		} else {
		...
		cdprec=(cdp_generic_t *)((uint8_t*)cdprec+ntohs(cdprec->length));
		/* printf("DEBUG: data = %p, cdprec = %p, cdprec-data = %u,"
			" length = %u\n",
			data,(uint8_t *)cdprec,((uint8_t*)cdprec-(uint8_t*)data),as);
			*/
	    } while ( ((uint8_t*)cdprec-(uint8_t*)data)<as);
